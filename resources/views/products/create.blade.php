...
@section('title', __('messages.product_form_title_create'))
...
<div class="max-w-2xl mx-auto bg-white p-6 md:p-8 rounded-2xl border border-gray-200 shadow-sm space-y-6">
    ...
    <div>
        <div class="flex justify-between items-center mb-3">
            ...
            <h2 class="text-sm md:text-base font-bold text-[#1B4D3E] tracking-wide">{{ __('messages.product_form_step_title') }}</h2>
            <div class="w-10"></div>
        </div>
    ...
    <div class="flex items-center gap-4 border border-emerald-700/30 bg-emerald-50/50 p-4 rounded-xl">
        ...
        <p class="text-xs text-[#1B4D3E] font-medium leading-relaxed">{{ __('messages.product_form_banner_text') }}</p>
    </div>
    ...
    <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <p class="text-[10px] text-gray-400 mt-1">{{ __('messages.product_form_name_help') }}</p>
        </div>
        ...
        <div>
            ...
            <input type="number" step="0.01" name="price" id="price" value="{{ old('price') }}" required min="0"
                ...
                placeholder="{{ __('messages.product_form_price_placeholder') }}">
            <p class="text-[10px] text-gray-400 mt-1">{{ __('messages.product_form_price_help') }}</p>
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" required
                class="w-full px-3 py-2.5 text-sm border border-emerald-700/40 rounded-xl focus:outline-none focus:border-[#1B4D3E] text-gray-800"
                placeholder="{{ __('messages.product_form_description_placeholder') }}">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="stock" id="stock" value="{{ old('stock') }}" required min="1"
                    class="w-full px-3 py-2.5 text-sm border border-emerald-700/40 rounded-xl focus:outline-none focus:border-[#1B4D3E] text-gray-800"
                    placeholder="{{ __('messages.product_form_stock_placeholder') }}">
            </div>
            ...
            <div>
                ...
                <select name="unit" id="unit" required
                    ...
                    <option value="Quintales" {{ old('unit') == 'Quintales' ? 'selected' : '' }}>{{ __('messages.unit_quintales') }}</option>
                    <option value="Libras" {{ old('unit') == 'Libras' ? 'selected' : '' }}>{{ __('messages.unit_libras') }}</option>
                    <option value="Cajas" {{ old('unit') == 'Cajas' ? 'selected' : '' }}>{{ __('messages.unit_cajas') }}</option>
                    <option value="Sacos" {{ old('unit') == 'Sacos' ? 'selected' : '' }}>{{ __('messages.unit_sacos') }}</option>
                    <option value="Unidades" {{ old('unit') == 'Unidades' ? 'selected' : '' }}>{{ __('messages.unit_unidades') }}</option>
                </select>
        ...
        <div>
            <label for="availability_date" class="block text-xs font-bold text-gray-700 mb-1">{{ __('messages.product_form_availability_label') }}</label>
        ...
        <!-- Foto principal -->
        <div>
            <label for="image" class="block text-xs font-bold text-gray-700 mb-1">{{ __('messages.product_form_image_label') }}</label>
        ...
        <div class="pt-4">
            <button type="submit" class="w-full py-3 bg-[#1B4D3E] hover:bg-[#14382c] text-white text-xs font-bold rounded-xl shadow-sm transition text-center tracking-wide">
                {{ __('messages.product_form_continue') }}
            </button>
        </div>
