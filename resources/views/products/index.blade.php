...
<div class="container mx-auto px-4 py-4 max-w-5xl space-y-6 md:space-y-8">
    ...
    <div class="flex justify-between items-center md:hidden">
        ...
        <a href="{{ route('profile.show') }}" class="flex flex-col items-end gap-1">
            <span class="flex items-center gap-1 text-[10px] font-bold text-gray-800">
                ...
                {{ Auth::user()->department ?? __('messages.dept_matagalpa') }}, Nicaragua
            </span>
    ...
    <div class="hidden md:flex justify-between items-center bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-[#1B4D3E]">{{ __('messages.welcome_title', ['name' => Auth::user()->name]) }}</h1>
                <p class="text-sm text-gray-600 mt-0.5">{{ __('messages.subtitle') }}</p>
            </div>
        </div>
        <div class="flex items-center gap-3 bg-gray-50 border border-gray-200 px-4 py-2.5 rounded-xl">
            ...
            <span class="text-sm font-bold text-gray-800">{{ Auth::user()->department ?? __('messages.dept_matagalpa') }}, Nicaragua</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-3 md:gap-4 max-w-2xl mx-auto">
        <a href="{{ route('products.my-products') }}" class="border border-gray-200 hover:border-[#1B4D3E] rounded-xl p-3 flex flex-col items-center justify-center text-center shadow-sm transition bg-white group">
            ...
            <span class="text-[10px] md:text-xs font-semibold text-gray-700 leading-tight">{{ __('messages.my_publications') }}</span>
        </a>
        ...
        <a href="{{ route('settings.index') }}" class="border border-gray-200 hover:border-[#1B4D3E] rounded-xl p-3 flex flex-col items-center justify-center text-center shadow-sm transition bg-white group">
            ...
            <span class="text-[10px] md:text-xs font-semibold text-gray-700 leading-tight">{{ __('messages.settings') }}</span>
        </a>
        ...
        <a href="{{ route('chats.index') }}" class="border border-gray-200 hover:border-[#1B4D3E] rounded-xl p-3 flex flex-col items-center justify-center text-center shadow-sm transition bg-white group">
            ...
            <span class="text-[10px] md:text-xs font-semibold text-gray-700 leading-tight">{{ __('messages.messages') }}</span>
        </a>
    </div>
    ...
    <div class="relative rounded-xl overflow-hidden bg-[#1B4D3E] text-white flex items-center h-28 md:h-36 shadow-sm">
        <div class="z-10 w-3/5 p-4 md:p-8">
            <h2 class="text-[13px] md:text-xl font-bold leading-tight">{{ __('messages.producer_banner') }}</h2>
        </div>
    ...
    <section>
        <h3 class="font-bold text-xs md:text-base text-gray-900 mb-3 md:mb-4">{{ __('messages.featured_categories') }}</h3>
    ...
    <section>
        <h3 class="font-bold text-xs md:text-base text-gray-900 mb-3 md:mb-4">{{ __('messages.featured_products') }}</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
            ...
            @empty
                <div class="col-span-full py-6 text-center text-xs md:text-sm text-gray-500 border border-dashed border-gray-300 rounded-xl">
                    {{ __('messages.products_no_products') }}
                </div>
            @endforelse
        </div>
    </section>
    ...
    <div class="border border-[#1B4D3E]/30 rounded-xl p-4 md:p-6 flex justify-between items-center bg-white shadow-sm gap-2">
        <div class="w-2/3 md:w-3/4">
            <h4 class="font-bold text-[11px] md:text-lg text-[#1B4D3E]">{{ __('messages.are_you_producer') }}</h4>
            <p class="text-[9px] md:text-sm text-gray-600 mt-0.5 leading-tight">{{ __('messages.producer_cta') }}</p>
        </div>
    ...
    <section>
        <div class="flex justify-between items-center mb-3 md:mb-4">
            <h3 class="font-bold text-xs md:text-base text-gray-900">{{ __('messages.productive_zones') }}</h3>
            <span class="text-[10px] md:text-xs text-[#1B4D3E] font-bold">{{ __('messages.products_zones_swipe') }}</span>
        </div>
    ...
    <div class="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 md:p-5 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h4 class="text-xs md:text-sm font-bold text-emerald-900">{{ __('messages.products_multilingual_title') }}</h4>
                <p class="text-[11px] md:text-xs text-emerald-700 mt-0.5">{{ __('messages.products_multilingual_text') }}</p>
            </div>
        </div>
