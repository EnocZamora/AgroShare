...
<body class="bg-gray-50 text-gray-800 min-h-screen flex flex-col pb-16 md:pb-0">
    ...
    <header class="bg-[#1B4D3E] text-white shadow-sm sticky top-0 z-50 hidden md:block">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex justify-between items-center">
            ...
            <nav class="flex items-center gap-6 font-medium text-sm">
                <a href="{{ route('products.index') }}" class="hover:text-emerald-200 transition {{ request()->routeIs('products.index') ? 'text-emerald-300' : '' }}">{{ __('messages.nav_home') }}</a>
                <a href="{{ route('products.create') }}" class="hover:text-emerald-200 transition {{ request()->routeIs('products.create') ? 'text-emerald-300' : '' }}">{{ __('messages.nav_publish') }}</a>
                <a href="{{ route('chats.index') }}" class="hover:text-emerald-200 transition {{ request()->routeIs('chats.*') ? 'text-emerald-300' : '' }}">{{ __('messages.nav_messages') }}</a>
                
                <!-- Selector de idioma en header desktop -->
                <div class="flex items-center gap-1.5 ml-4 border-l border-white/20 pl-4">
                    <form action="{{ route('lang.switch', 'es') }}" method="GET" class="inline">
                        <button type="submit" class="px-2 py-1 text-xs font-medium rounded {{ session('locale', 'es') === 'es' ? 'bg-white/30 text-white' : 'text-white/70 hover:text-white' }} transition">
                            ES
                        </button>
                    </form>
                    <form action="{{ route('lang.switch', 'mi') }}" method="GET" class="inline">
                        <button type="submit" class="px-2 py-1 text-xs font-medium rounded {{ session('locale') === 'mi' ? 'bg-white/30 text-white' : 'text-white/70 hover:text-white' }} transition">
                            MI
                        </button>
                    </form>
                    <form action="{{ route('lang.switch', 'cr') }}" method="GET" class="inline">
                        <button type="submit" class="px-2 py-1 text-xs font-medium rounded {{ session('locale') === 'cr' ? 'bg-white/30 text-white' : 'text-white/70 hover:text-white' }} transition">
                            CR
                        </button>
                    </form>
                </div>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 py-2 px-4 flex justify-between items-center z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <a href="{{ route('products.index') }}" class="flex-1 flex flex-col items-center gap-1 {{ request()->routeIs('products.index') ? 'text-[#1B4D3E] font-bold' : 'text-gray-500 hover:text-gray-700' }}">
            ...
            <span class="text-[10px]">{{ __('messages.nav_home') }}</span>
        </a>
        <a href="{{ route('products.create') }}" class="flex-1 flex flex-col items-center gap-1 {{ request()->routeIs('products.create') ? 'text-[#1B4D3E] font-bold' : 'text-gray-500 hover:text-gray-700' }}">
            ...
            <span class="text-[10px]">{{ __('messages.nav_publish') }}</span>
        </a>
        <a href="{{ route('chats.index') }}" class="flex-1 flex flex-col items-center gap-1 {{ request()->routeIs('chats.*') ? 'text-[#1B4D3E] font-bold' : 'text-gray-500 hover:text-gray-700' }}">
            ...
            <span class="text-[10px]">{{ __('messages.nav_messages') }}</span>
        </a>
        <a href="{{ route('profile.show') }}" class="flex-1 flex flex-col items-center gap-1 text-gray-500 hover:text-gray-700">
            ...
            <span class="text-[10px]">{{ __('messages.nav_profile') }}</span>
        </a>
    </nav>
